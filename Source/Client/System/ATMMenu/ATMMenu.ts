...
API.onServerEventTrigger.connect(function (eventName, args) {
	if (eventName == "ATM_OpenMenu") {
        ...
        API.setMenuSubtitle(ATM_Menu, "~b~Kontostand~w~: " + ATM_Player_Bank + " ~g~$")
	}
...
var ATM_AmountItem = API.createMenuItem("Betrag", "");
var ATM_WithdrawItem = API.createMenuItem("~b~Abheben", ""); // Bank => Cash
var ATM_DepositItem = API.createMenuItem("~g~Einzahlen", ""); // Cash => Bank
var ATM_Amount = "0";
	...
	switch (item) {
		case ATM_AmountItem:
			...
			if (ATMisValid) {
                ...
                API.sendNotification("~r~Error~w~: Bitte geben Sie eine gültige Nummer ein")
				ATM_Amount = "0";
				ATM_AmountItem.SetRightLabel(ATM_Amount + ".00 ~g~$");
			}
			break;
