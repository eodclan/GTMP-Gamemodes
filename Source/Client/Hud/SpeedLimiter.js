...
API.onKeyDown.connect(function(sender, key) {
    if (!API.isChatOpen() && key.KeyCode == Keys.L && API.isPlayerInAnyVehicle( API.getLocalPlayer() ) && API.getPlayerVehicleSeat( API.getLocalPlayer() ) == -1)
    {
        ...
        {
            API.sendNotification("~r~Speed Limiter kann an diesem Fahrzeug nicht verwendet werden!");
            return;
        }
        ...
        if (limitMenu == null) {
            ...
            limitSpeedItem = API.createListItem("Limit", "Passt die Geschwindigkeitsbegrenzung an.", limits, 0);
            limitMenu.AddItem(limitSpeedItem);

            limitToggleItem = API.createCheckboxItem("Aktiv", "Schaltet das Geschwindigkeitslimit um.", (vehicleMaxSpeedEnabled[model] !== undefined) ? vehicleMaxSpeedEnabled[model] : false);
