			...
			API.shared.getAllPlayers().ForEach(otherclient => {
				if (otherclient.hasData("player"))
				{
					...
					if(otherplayer.Character.Faction == FactionType.Sheriff && otherplayer.Character.OnDuty)
					{
						if (onDuty)
						{
							API.shared.sendNotificationToPlayer(otherclient, $"~y~Sheriff Information:~n~~b~{player.Character.FirstName} {player.Character.LastName} ~w~ist jetzt ~g~im Dienst");
						}
						else
						{
							API.shared.sendNotificationToPlayer(otherclient, $"~y~Sheriff Information:~n~~b~{player.Character.FirstName} {player.Character.LastName} ~w~ist jetzt ~g~ausser Dienst");
						}
					}
				}
			});
