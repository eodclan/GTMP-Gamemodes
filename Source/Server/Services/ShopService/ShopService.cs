...
namespace FactionLife.Server.Services.ShopService
{
		...
		public static void LoadShopsFromDB()
		{
			...
			if (result.Rows.Count != 0)
			{
				foreach (DataRow row in result.Rows)
				{
                    ...
                    shop.Blip.sprite = 52;
                    shop.Blip.scale = 0.8f;
                    shop.Blip.name = "Shop";
                    shop.Blip.shortRange = true;
                    BlipService.BlipService.BlipList.Add(shop.Blip);

                    shop.Ped = API.shared.createPed(PedHash.AmandaTownley, shop.PedPosition, shop.PedRotation.Z);

                    ShopList.Add(shop);
                }
				API.shared.consoleOutput(LogCat.Info, result.Rows.Count + " Geschäfte geladen..");
			}
			else
			{
				API.shared.consoleOutput(LogCat.Info, "Geschäfte Nicht geladen..");
			}
		}
