...
namespace FactionLife.Server.Services.PhoneService
{
    /// <summary>
    ...
    public class Phone
    {
        ...
        public Phone(Character c, String Phone)
        {
            owner = c;
            this.Phone = Phone;
        ...
        private void AddReceivedTextMessageToDatabase(TextMessage message)
        {
			...
			DataTable result = DatabaseHandler.ExecutePreparedStatement("INSERT INTO text_messages VALUES (@id, @sender_number, @receiver_number, @time, @message)", parameters);				
        }
        ...
        private void RemoveTextMessageFromDatabase(int id)
        {
			...
			DataTable result = DatabaseHandler.ExecutePreparedStatement("DELETE FROM text_messages WHERE id=@id", parameters);					
        }
        ...
        private void AddContactToDatabase(Address address)
        {
			...
			DataTable result = DatabaseHandler.ExecutePreparedStatement("INSERT INTO phone_contacts VALUES (@owner, @name, @number)", parameters);					
        }
        ...
        private void RemoveContactFromDatabase(String number)
        {
			...
			DataTable result = DatabaseHandler.ExecutePreparedStatement("DELETE FROM phone_contacts WHERE owner=@owner AND number=@number", parameters);			
        }
        ...
        {
            TextMessage message;
            message.senderNumber = this.Phone;
        ...
        public void SetCallActive(String number)
        {
            ...
            callerNumber = number;
            activeCallPhone = PlayerManager.Instance().GetCharacterWithPhone(number).phone;
        }
        ...
        {
            Character characterToCall = PlayerManager.Instance().GetCharacterWithPhone(number);
            ...
            else
            {
                ...
                else
                {
                    ...
                    characterToCall.phone.ReceiveCall(this.Phone);
        ...
        public void PickupCall()
        {
            ...
            {
                Character characterCaller = PlayerManager.Instance().GetCharacterWithPhone(this.callerNumber);
                if (characterCaller != null)
                {
                    API.shared.consoleOutput("Pickup call");
                    if (characterCaller.phone.isCalling)
                    {
                        characterCaller.phone.SetCallActive(this.Phone);
        ...
        public void HangUpCall()
        {
            ...
            if (this.isCalling)
            {
                Character callingChar = PlayerManager.Instance().GetCharacterWithPhone(this.callingNumber);
