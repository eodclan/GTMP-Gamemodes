			...
			API.shared.getAllPlayers().ForEach(otherclient => {
				if (otherclient.hasData("player"))
				{
					...
					if (otherplayer.Character.Faction == FactionType.EMS && otherplayer.Character.OnDuty)
					{
						if (onDuty)
						{
							API.shared.sendNotificationToPlayer(otherclient, $"~y~EMS Information:~n~~b~{player.Character.FirstName} {player.Character.LastName} ~w~~ist jetzt ~g~im Dienst");
						}
						else
						{
							API.shared.sendNotificationToPlayer(otherclient, $"~y~EMS Information:~n~~b~{player.Character.FirstName} {player.Character.LastName} ~w~ist jetzt ~g~ausser Dienst");
						}
					}
				}
			});
