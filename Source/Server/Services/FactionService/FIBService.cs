...
namespace FactionLife.Server.Services.FactionService
{
		...
		: Script
	{
			...
			API.shared.getAllPlayers().ForEach(otherclient => {
				if (otherclient.hasData("player"))
				{
					...
					if(otherplayer.Character.Faction == FactionType.FIB && otherplayer.Character.OnDuty)
					{
						if (onDuty)
						{
                            API.shared.sendPictureNotificationToPlayer(otherclient, "Herr " + player.Character.FirstName + " " + player.Character.LastName + " ~w~ist jetzt ~g~im Dienst ~w~gegangen", "CHAR_ACTING_UP", 0, 2, "Dienststelle", "FIB Information");
                        }
                        else
                        {
                            API.shared.sendPictureNotificationToPlayer(otherclient, "Herr " + player.Character.FirstName + " " + player.Character.LastName + " ~w~ist jetzt ~g~ausser Dienst ~w~gegangen", "CHAR_ACTING_UP", 0, 2, "Dienststelle", "FIB Information");
                        }
					}
				}
			});
		...
		{
			API.shared.triggerClientEvent(client, "FIB_CloseInteractionMenu");
		}

		#region FIB Interaction Menu
		...
		#endregion FIB Interaction Menu

		#region FIB Weapon Menu
        ...
        public static void ProcessWeaponMenu(Client client, string itemvalue)
        {
            if (!client.hasData("player")) { return; }
            Player player = client.getData("player");
            if (player.Character.Faction != FactionType.FIB || !player.Character.OnDuty) { return; }
            ...
            API.shared.triggerClientEvent(client, "FIB_CloseWeaponMenu");
        }
        ...
        public static void OpenWeaponMenu(Client client)
		{
			...
			API.shared.triggerClientEvent(client, "FIB_OpenWeaponMenu", JsonConvert.SerializeObject(BuildWeaponMenu(player)));
		}
		#endregion FIB Weapon Menu
