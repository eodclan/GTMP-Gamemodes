...
namespace FactionLife.Server.Services.FactionService
{
		...
		: Script
	{
		...
		{
			InterfaceService.ScreenService.PlayScreenEffect(client, "DeathFailMPDark", 10000, true);
			...
			if (IsMedicOnline()){
				...
				blip.sprite = 305;
				blip.color = 1;
				blip.name = $"Dispatch: In Lebensgefahr! | {player.Character.FirstName} {player.Character.LastName}";
				Dispatches.Add(
					new Dispatch
					{
						Client = client,
			...
			{
				StartDeathTimer(client, 120); // No Medic is Online Respawn in 30 Seconds
			}
		...
		public static void SpawnAtHospital(Client client)
		{
			...
			API.shared.delay(2000, true, () => {
				client.position = new Vector3(-457.8979, -327.3404, 34.50191);
		...
		public static void SetOnDuty(Client client, bool onDuty)
		{
			...
			if (onDuty)
			{
				...
				{
					ClothingService.ClothingService.ApplyOutfit(client, 14); // Male EMS Outfit
				}
				else
				{
					ClothingService.ClothingService.ApplyOutfit(client, 16); // Female EMS Outfit
				}
			}
			else
			{
			...
			API.shared.getAllPlayers().ForEach(otherclient => {
				if (otherclient.hasData("player"))
				{
					...
					if (otherplayer.Character.Faction == FactionType.EMS && otherplayer.Character.OnDuty)
					{
                        if (onDuty)
                        {
                            API.shared.sendPictureNotificationToPlayer(otherclient, "Herr " + player.Character.FirstName + " " + player.Character.LastName + " ~w~ist jetzt ~g~im Dienst ~w~gegangen", "CHAR_CASTRO", 0, 2, "Dienststelle", "EMS Information");
                        }
                        else
                        {
                            API.shared.sendPictureNotificationToPlayer(otherclient, "Herr " + player.Character.FirstName + " " + player.Character.LastName + " ~w~ist jetzt ~g~ausser Dienst ~w~gegangen", "CHAR_CASTRO", 0, 2, "Dienststelle", "EMS Information");
                        }
                    }
				}
			});
