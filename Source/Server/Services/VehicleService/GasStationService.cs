	...
	class GasStationService
	{
		...
		public static void LoadAllGasStationsFromDB()
		{
			...
			if (result.Rows.Count != 0)
			{
				...
				API.shared.consoleOutput(LogCat.Info, result.Rows.Count + " Tankstellen geladen..");
			}
			else
			{
				API.shared.consoleOutput(LogCat.Info, "Keine Tankstellen geladen..");
			}
		}
		...
		public static void OpenGasStationMenu(Client client)
		{
			...
			if (foundGasPump)
			{
                ...
                // Benzin
                if (gasStation.StationFuelPrices.Benzin != 0)
				{
					gasItem = new GasStationMenuItem
					{
						Name = FuelType.Benzin.ToString(),
						RightLabel = gasStation.StationFuelPrices.Benzin + " ~g~$",
						Ident = (int)FuelType.Benzin
                    };
					if (gasStation.Storage.Benzin <= 0) { gasItem.SoldOut = true; } else { gasItem.SoldOut = false; }
		...
		public static void StartRefill(Client client, OwnedVehicle ownedVehicle, FuelType fuelType)
		{
			...
			{
				API.shared.sendNotificationToPlayer(client, "~r~Sie müssen zuerst den Motor abstellen, bevor Sie mit dem Tanken beginnen.");
				return;
			}
			GasStation gasStation = GasStationList.FirstOrDefault(x => x.Position.DistanceTo(client.position) <= 20);
			...
			{
				API.shared.sendNotificationToPlayer(client, "~o~Du willst nicht den falschen Treibstoff Tanken " + fuelType.ToString() + " in dein " + ownedVehicle.ModelName + "(" +
					VehicleService.GetVehicleInfo(ownedVehicle.ModelName).Fuel.ToString() + ").. ~r~~n~Erinnere dich daran..");
				return;
			}
			...
			switch (fuelType)
			{
				case FuelType.Benzin:
					price = gasStation.StationFuelPrices.Benzin;
					break;
			...
			Timer timer = API.shared.startTimer(1000, false, () => {
				if(ownedVehicle.Fuel < VehicleService.GetVehicleInfo(ownedVehicle.ModelName).MaxFuel)
				{
					...
					{
						case FuelType.Benzin:
							if (gasStation.Storage.Benzin > 0)
							{
								gasStation.Storage.Benzin--;
							}
							else
							{
								StopRefill(client);
								API.shared.sendNotificationToPlayer(client, "~o~Diese Tankstelle hat nicht mehr ~b~Benzin~o~ auf Lager..");
								return;
							}
							break;
						case FuelType.Diesel:
							...
							{
								StopRefill(client);
								API.shared.sendNotificationToPlayer(client, "~o~Diese Tankstelle hat nicht mehr ~b~Diesel~o~ auf Lager..");
								return;
							}
							break;
						case FuelType.Gas:
							...
							{
								StopRefill(client);
								API.shared.sendNotificationToPlayer(client, "~o~Diese Tankstelle hat nicht mehr ~b~Gas~o~ auf Lager..");
								return;
							}
							break;
					}
					...
					{
						StopRefill(client);
						API.shared.sendNotificationToPlayer(client, "~r~Sie haben nicht genug Geld, um den Tanken fortzusetzen.");
						return;
					}
		...
		public static void StopRefill(Client client)
		{
			...
			API.shared.sendNotificationToPlayer(client, "~b~Auftanken gestoppt~n~~w~Du hast bezahlt ~g~" + (fueled * GetFuelPrice(gasStation ,fuelType)) + "$");
