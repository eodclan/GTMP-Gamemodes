	...
	class VehicleShopService
	{
		...
		public static void LoadAllVehicleShopsFromDB()
		{
			...
			if (result.Rows.Count != 0)
			{
				...
				API.shared.consoleOutput(LogCat.Info, result.Rows.Count + " Fahrzeuggeschäfte geladen..");
			}
			else
			{
				API.shared.consoleOutput(LogCat.Info, "Keine Fahrzeuggeschäfte geladen..");
			}
		}
		...
		public static void OpenVehicleShop(Client client)
		{
			...
			{
				API.shared.sendPictureNotificationToPlayer(client, "Entschuldigung, wir verkaufen im Moment nichts..", "CHAR_BLOCKED", 0, 0, "ID: ~b~" + vehicleShop.Id, "Vehicle Shop");
				return;
			}
        ...
        public static void BuyVehicle(Client client, string modelName)
		{
			...
			if (vehicleShop == null)
			{
				API.shared.sendNotificationToPlayer(client, "~o~Fahrzeug - Shop konnte nicht gefunden werden..");
				CloseVehicleShop(client);
				return;
			}
			if (!vehicleShop.SellingVehicles.ContainsKey(modelName))
			{
				API.shared.sendNotificationToPlayer(client, "~o~Das angeforderte Fahrzeug konnte nicht gefunden werden..");
				CloseVehicleShop(client);
				return;
			}
			if (!MoneyService.MoneyService.HasPlayerEnoughBank(client, vehicleShop.SellingVehicles[modelName]))
			{
				API.shared.sendNotificationToPlayer(client, "~r~Du hast nicht genug Geld!");
				CloseVehicleShop(client);
				return;
			}
			...
			OwnedVehicle ownedVehicle = new OwnedVehicle {
				...
				EngineHealth = 1000,
				SecondaryColor = 1,
                NumberPlate = "LS-" + GetTimestamp(DateTime.Now),
			...
			API.shared.sendNotificationToPlayer(client, "~g~Du hast eine gekauft ~b~" + modelName + "~g~ für ~b~" + vehicleShop.SellingVehicles[modelName] + "$~n~" +
                "~o~Ihr Fahrzeug Ist nun in der Garage.");
			CloseVehicleShop(client);
		}
