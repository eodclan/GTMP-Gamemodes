...
API.onResourceStart.connect(function () {
    ShopBuyMenu = API.createMenu(" ", "Einkaufen", 0, 0, 6);
    ShopSellMenu = API.createMenu(" ", "Verkaufen", 0, 0, 6);
	...
	ShopSellItem = API.createMenuItem("Verkaufen", "");
...
function FillShopSellMenu() {
	...
	for (var i = 0; i < ShopSellJson.length; i++) {
		...
		if (ShopInvObj["Count"] != 0) {
            var NewItem = API.createMenuItem(ShopInvObj["Name"], "~b~jeder ~w~" + ShopInvObj["Description"] + " ~g~$");
