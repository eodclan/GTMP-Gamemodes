...
var VehicleShopMenu = API.createMenu("Fahrzeug Händler", "Verfügbare Fahrzeuge:", 0, 0, 6);
...
function VehicleShopInfoPanel(title, subtitle, row1, row2, vehHash) {
    ...
    info_panel.CallFunction("SET_VEHICLE_INFOR_AND_STATS", title, "Treibstoffart: " + row1 + " | Lager: " + row2, "MPCarHUD", "Cheval", "Top Speed", "Bremsen", "Traktion", "Beschleunigung", Math.round(API.getVehicleMaxSpeed(vehHash) * 1.2), Math.round(API.getVehicleMaxBraking(vehHash) * 40), Math.round(API.getVehicleMaxTraction(vehHash) * 20), Math.round(API.getVehicleMaxAcceleration(vehHash) * 40));
